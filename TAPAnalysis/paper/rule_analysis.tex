...
\end{enumerate}
\todo{finish this section, lots to add-- use of homecfg, what happens in case of time and system triggers only?--suggestions are not useful and missing triggers are identified if rule also has atleast some event based trigger. How to eliminate triggers that dont make sense? if it is not modified before reading from, then it is not useful--how did you implement this?}
\begin{figure*}
\centering
