...
\emph{Approach}. Our approach is to automatically generate all the correct trigger conditions from the actions written by the end users. We developed a static technique to achieve this so that the rules do not need to be fired at all until all the correct triggers are included. Figure~\ref{fig:design} shows the design of our tool. It takes as input a \textit{rules} file and and \textit{items} file. In the first step, we manually generate a \textit{homecfg} configuration file based on the items file and the core actions supported by the openHAB runtime. This configuration file contains two types of information:
\begin{enumerate} [topsep=0pt,itemsep=-1ex]
\item names of those actions that do not affect the state of any device, e.g. logging actions. 
\item item names that represent output states.
\end{enumerate}
\todo{finish this section, lots to add-- use of homecfg, what happens in case of time and system triggers only?--no suggestions and missing triggers identified if rule also has atleast some event based trigger. How to eliminate triggers that dont make sense? if it is not modified before reading from, then it is not useful--how did you implement this?}
\begin{figure*}
\centering
...
We used the AST generated by xtext for the rule \textit{scripts} and visited the nodes in the tree to extract relevant information such as the names of the device states used in the method calls or in conditional statements. \todo{[do people care to know about the details?]}
 
\section{Experimental Evaluation}
We evaluated our tool on 116 end user written rules. We obtained the rules from links provided on the openHAB wiki~\cite{data1},\cite{data2}. It took $\sim$ 7.12 ms for the tool to generate the triggers for all the rules. Tables~\ref{tab:results1} and \ref{tab:results2} show the summary of our results.
\begin{table*}
\centering
\begin{tabular}{|c|c|c|c|}
\hline
Rule name & \# suggested triggers & \# user written triggers & \# missing triggers \\ \hline
Rule name & \# suggested triggers & \# user written triggers & \# missing triggers \\ \hline
Rule name & \# suggested triggers & \# user written triggers & \# missing triggers \\ \hline
Rule name & \# suggested triggers & \# user written triggers & \# missing triggers \\ \hline
Rule name & \# suggested triggers & \# user written triggers & \# missing triggers \\ \hline
...
Rule name & \# suggested triggers & \# user written triggers & \# missing triggers \\ \hline
\end{tabular}
\caption{Summary of results for rules for which triggers were suggested.}
\label{tab:results1}
\end{table*}
...
Rule name & \# suggested triggers & \# user written triggers & \# missing triggers \\ \hline
\end{tabular}
\caption{Summary of results for rules for which missing triggers were detected.}
\label{tab:results2}
\end{table*}
