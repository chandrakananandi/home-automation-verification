...
\item Naively adding all of the potential triggers would unnecessarily fire the rules too many time. Hence, we apply an elimination algorithm which has three steps.
\begin{itemize}
...
\item item names that represent output states. This refers to states that do not change the state of any device in the house. Examples would be items meant for notifications or status messages. 
\end{enumerate}
\end{itemize}
\end{enumerate}
