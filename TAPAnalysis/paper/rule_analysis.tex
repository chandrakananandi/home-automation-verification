...
\item Naively adding all $t \in T$ as triggers of $R$ would either unnecessarily fire $R$ too many time or add redundant triggers which would make $R$ unnecessarily complicated. Hence, we apply an elimination algorithm which has two steps. The following two definitions are used in the elimination algorithm. 

\emph{Dependent item}. We define an item to be \emph{dependent} if its value is computed based on other items and it is not live at any point in a rule, i.e its value is not read before it is written to.

\emph{Independent item}. We define an item to be \emph{independent} if its value is directly obtained from a sensor or a user's input.

\emph{Redundant trigger}. We define an event-based trigger to be redundant if inclusion of the trigger in a rule \emph{never} changes the state of any item involved in the rule when the rule is fired due to it.
\begin{enumerate}
\item  A dependent item $d$ is not included in $T$ because it is redundant.  \\
\emph{Theorem}. A dependent item $d$ is a redundant trigger.\\
\emph{Proof}. Let us prove this by contradiction and assume that $d$ is not a redundant trigger. Since $d$ is dependent, it means that $d$'s value is computed based on other items and $d$ is never read before being written to. Let $i_1, i_2, ..., i_n, n > 0$ be the other items involved in $R$ and let $i_1, i_2, ..., i_m, m \leqslant n$ be those on which $d$ depends: $d = f (i_1, i_2, ..., i_m)$. Without loss of generality, we assume $i_1, i_2, ..., i_m$ to be independent.
If $d$ is not redundant, then in some firing of $R$ due to $d$, the value of at least one of $d, i_1, i_2, ..., i_n$ must change. Since $d$ is never read from before being written to, none of $i_1, i_2, ..., i_n$ can be affected by $d$. Further, since $d$ only depends on $i_1, i_2, ..., i_m$ and not on itself, $d$ as a trigger will never change its own value. Thus, $d$ never changes the value of any of the involved items in R. This contradicts our assumption that $d$ is non-redundant and we thus conclude that $d$ must be redundant. $\blacksquare$


\item We manually generate a \textit{homecfg} configuration file based on the items file and the core actions supported by the openHAB runtime as described in section~\ref{subsec:actions}. This configuration file contains two types of information:
\begin{enumerate}
